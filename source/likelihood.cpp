...
//                + 0.5*(1/(sd_prior_alpha*sqrt(2*M_PI)))*exp(-(alpha-m2_prior_alpha)*(alpha-m2_prior_alpha)/(2*sd_prior_alpha*sd_prior_alpha)) );
  //return log_sigma_prior_alpha - ((alpha*alpha)/(2*sd_prior_alpha*sd_prior_alpha));	//revision env1 (PdV) --> calculate the log only once, and never after
  return log_sigma_prior_alpha - (((alpha-mean_prior_alpha)*(alpha-mean_prior_alpha))/(2*sd_prior_alpha*sd_prior_alpha));
}

//revision env1 (PdV)
double log_prior_g(double g)
{
// 	double log_g;	
// 	log_g=log(g);	//Calculate the log only once during the function
// 	return log_sigma_prior_g - log_g - ((log_g*log_g)/(2*sd_prior_g*sd_prior_g));
	return -log(U);
}
...
{
    double loglikelihood=0;
    double pf;			//revision env1: changed g to pf to avoid confusion (PdV)
    double theta;

    #pragma omp parallel for SCHED_I reduction(+:loglikelihood) private(pf, theta)
    for (int i=0;i<I;i++)
    {
        if (!discarded_loci[i])
        {
            ...
            for (int j=0;j<J;j++)
            {
                ...
                {
                    pf= pop[j].locus[i].p*pop[j].locus[i].p
                    ...
                    loglikelihood += factln(pop[j].locus[i].n)
                                     ...
                                     +pop[j].locus[i].nA1*log(pf)
                                     +(pop[j].locus[i].n-pop[j].locus[i].nA1)*log(1-pf);
                }
