...
#define sd_prior_beta 1 // 1 sd of the normal prior for beta
#define mean_prior_beta -1 // -1 mean of the normal prior for bet

#define lambda 1 // parameter for the non informative dirichlet prior of allele frequencies

#define var_intensity 10 // parameter for the variance between delta and mu //revision env1 (Pdv) --> gamma changed to var_intensity to avoid conflicts
...
void update_ancestral_freq()
{
    ...
    double BwPrp,FwPrp; // forward and backward proposition probability
    ...
    for (int i=0; i<I; i++) // cycle over loci
    {
        if (!discarded_loci[i])
        {
            ...
            FwPrp=min(1.0-epsilon,old_freq+e_ancestral[i])-max(0.0+epsilon,old_freq-e_ancestral[i]);

            // propose the new values
            u=randgen_parallel[omp_get_thread_num()].randDblExc(FwPrp)+max(0.0+epsilon,old_freq-e_ancestral[i]);
            if (u<=epsilon) u=epsilon;
            if (u>=1-epsilon) u=1-epsilon;
            freq_ancestral[i]=u;

            // calculate backward proposition probability =q(p,p')
            BwPrp=min(1.0-epsilon,freq_ancestral[i]+e_ancestral[i])-max(0.0+epsilon,freq_ancestral[i]-e_ancestral[i]);

            // ratio A
            A= log(FwPrp) - log(BwPrp)
               + (a_p-1)*log((1-freq_ancestral[i])/(1-old_freq))+(a_p-1)*log(freq_ancestral[i]/old_freq);
            double theta;
            for (int j=0; j<J; j++)
            {
                theta=exp(-(alpha[i]+beta[j]+g[i]*X[j]));				//revision env1 (PdV)
...
void update_freq()
{
    ...
    double A; // log of A in MH algorithm : accept move with probability min(1,A)
    double BwPrp,FwPrp; // forward and backward proposition probability
    double BwPrp2,FwPrp2; // forward and backward proposition probability
...
//    double old_log_likelihood; // old logikelihood
    ...
    double old_pf,new_pf; // store new and old phenotype frequency //revision env1: changed old_g and new_g to old_pf and new_pf to avoid confusion (PdV)

    double u; // proposal random value

    #pragma omp parallel for SCHED_J reduction(+:log_likelihood) private(r, A, FwPrp, BwPrp, FwPrp2, BwPrp2, diff_log_likelihood, old_freq, old_pf, new_pf, u)
    for (int j=0; j<J; j++) //cycle over populations
    {
        for (int i=0; i<I; i++) // cycle over loci
        {
            if (!discarded_loci[i])
            {
                ...
                old_pf= pop[j].locus[i].p*pop[j].locus[i].p
                        +2*pop[j].locus[i].p*(1-pop[j].locus[i].p)*(1-f[j])
                        +f[j]*pop[j].locus[i].p*(1-pop[j].locus[i].p);

                // calculate forward proposition probability =q(p',p)
                FwPrp=min(1.0-epsilon,old_freq+e_freq[i][j])-max(0.0+epsilon,old_freq-e_freq[i][j]);
                FwPrp2=min(1.0-epsilon,old_freq+e_small)-max(0.0+epsilon,old_freq-e_small);
                // propose the new values
                if (randgen_parallel[omp_get_thread_num()].rand()<1-prop_small)
                    u=randgen_parallel[omp_get_thread_num()].rand(FwPrp)+max(0.0+epsilon,old_freq-e_freq[i][j]);
                else
                    u=randgen_parallel[omp_get_thread_num()].rand(FwPrp2)+max(0.0+epsilon,old_freq-e_small);
                ...
                // calculate backward proposition probability =q(p,p')
                BwPrp=min(1.0-epsilon,pop[j].locus[i].p+e_freq[i][j])-max(0.0+epsilon,pop[j].locus[i].p-e_freq[i][j]);
                BwPrp2=min(1.0-epsilon,pop[j].locus[i].p+e_small)-max(0.0+epsilon,pop[j].locus[i].p-e_small);
                // store the old logikelihood and calculate the new logikelihood
                //old_log_likelihood=log_likelihood;

                new_pf= pop[j].locus[i].p*pop[j].locus[i].p
                        +2*pop[j].locus[i].p*(1-pop[j].locus[i].p)*(1-f[j])
                        +f[j]*pop[j].locus[i].p*(1-pop[j].locus[i].p);

                diff_log_likelihood= pop[j].locus[i].nA1*(log(new_pf)-log(old_pf))
                                     + (pop[j].locus[i].n-pop[j].locus[i].nA1)*(log(1-new_pf)-log(1-old_pf));

                //#pragma omp critical(likelihood)
                //log_likelihood=old_log_likelihood+diff_log_likelihood;

                double theta=exp(-(alpha[i]+beta[j]+g[i]*X[j]));				//revision env1 (PdV)
                // ratio A
                A = diff_log_likelihood
                    ...
                    + log((1-prop_small)*FwPrp+prop_small*FwPrp2) - log((1-prop_small)*BwPrp+prop_small*BwPrp2);
...
void update_f_random()
{
    double r; // random value to accept/reject the move
    double A; // log of A in MH algorithm : accept move with probability min(1,A)
    double BwPrp,FwPrp; // inverse of forward and backward proposition probability =1/q
    ...
    if (prior_fis_unif)
    {
        ...
        for (int j=0; j<J; j++) // cycle over populations
        {
            ...
            old_f=f[j];
            // calculate forward proposition probability =q(p',p)
            FwPrp=min(prior_fis_hb,old_f+e_f[j])-max(prior_fis_lb,old_f-e_f[j]);
            // propose the new values
            u=randgen_parallel[omp_get_thread_num()].randDblExc(FwPrp)+max(prior_fis_lb,old_f-e_f[j]);
            f[j]=u;
            // calculate backward proposition probability =q(p,p')
            BwPrp=min(1.0,f[j]+e_f[j])-max(0.0,f[j]-e_f[j]);
            // ratio A
            A= log(FwPrp) - log(BwPrp);
            r=randgen_parallel[omp_get_thread_num()].randDblExc();
            // reject proposed value
    ...
    else   // beta prior
    {
        ...
        for (int j=0; j<J; j++) // cycle over populations
        {
            ...
            old_f=f[j];
            // calculate forward proposition probability =q(p',p)
            FwPrp=min(1.0,old_f+e_f[j])-max(0.0,old_f-e_f[j]);
            // propose the new values
            u=randgen_parallel[omp_get_thread_num()].rand(FwPrp)+max(0.0,old_f-e_f[j]);
...
void update_beta()
{
    ...
    for (int j=0; j<J; j++) // cycle over populations
    {
		...
		if (is_pilot) {
		  new_theta=exp(-(beta[j]));
		  old_theta=exp(-(old_beta));
		} else {
		  new_theta=exp(-(alpha[i]+beta[j]+g[i]*X[j]));
		  old_theta=exp(-(alpha[i]+old_beta+g[i]*X[j]));
		}
...
//        A=log_prior_alpha(alpha[i])-log_prior_alpha(old_alpha);
            ...
            {
                // calculate old and new value of theta
                new_theta=exp(-(alpha[i]+beta[j]));				//revision env1 (PdV)
                old_theta=exp(-(old_alpha+beta[j]));				//revision env1 (PdV)
...
void update_g()
{
    double r; // random value to accept/reject the move
    double A; // log of A in MH algorithm : accept move with probability min(1,A)
    double BwPrp,FwPrp;		//Inverse of PDF for Backward and Forward jump
    double old_g; // store old allele freq if the move is rejected

    double log_old_g,log_g;	//log of old_g and g in case of log_normal prior 	//revision env1 (PdV)
    ...
    for (int i=0; i<I; i++)
    {
        if (!discarded_loci[i] && g_included)
        {
            ...
            // make the move
            //std::cout << omp_get_thread_num();
	    FwPrp=min(U,old_g+e_g[i])-max(0.0,old_g-e_g[i]);

            g[i]=randgen_parallel[omp_get_thread_num()].randDblExc(FwPrp)+max(0.0,old_g-e_g[i]);
            BwPrp=min(U,g[i]+e_g[i])-max(0.0,g[i]-e_g[i]);

            // ratio A
	    A= log(FwPrp) - log(BwPrp);		//log(odd_prior) for uniform is zero

            for (int j=0; j<J; j++)
            {
                // calculate old and new value of theta
                new_theta=exp(-(beta[j]+g[i]*X[j]));
                old_theta=exp(-(beta[j]+old_g*X[j]));
            ...
            if (log(r)>A)
            {
                g[i]=old_g;// restore old value of g
            }
            else
            {
                acc_g[i]++;
...
//double log_likelihood2;
    ...
    for (int i=0; i<I; i++) // cycle over loci
    {
        if (!discarded_loci[i])
        {
            ...
            FwPrp=min(old_m+old_n,old_m+e_ancestral[i])-max(0.0,old_m-e_ancestral[i]);

            // propose the new values
            u=randgen_parallel[omp_get_thread_num()].randDblExc(FwPrp)+max(0.0,old_m-e_ancestral[i]);
            freq_locus[i].allele[m]=u;
            freq_locus[i].allele[n]=old_n+(old_m-freq_locus[i].allele[m]);

            BwPrp=min(freq_locus[i].allele[m]+freq_locus[i].allele[n],freq_locus[i].allele[m]+e_ancestral[i])-max(0.0,freq_locus[i].allele[m]-e_ancestral[i]);
            ...
            // ratio A
            A= -old_l+new_l + log(FwPrp) - log(BwPrp)
...
void update_alpha_codominant()
{
    ...
    for (int i=0; i<I; i++)
    {
        ...
        if (!discarded_loci[i] && alpha_included[i])
        {
            ...
            // ratio A
            A= diff_log_likelihood 
            //+(old_alpha*old_alpha-alpha[i]*alpha[i])/(2*sd_prior_alpha*sd_prior_alpha);
            + ((old_alpha-alpha[i])*(old_alpha+alpha[i]-2*mean_prior_alpha))/(2*sd_prior_alpha*sd_prior_alpha);
...
void update_g_codominant()
{
    ...
    #pragma omp parallel for SCHED_I /*reduction(+:alpha_updates)*/ private(r, A, old_g, e,new_theta,old_theta,diff_log_likelihood,FwPrp,BwPrp)
    for (int i=0; i<I; i++)
    {
        if (!discarded_loci[i] && g_included[i])
        {
            ...
            // calculate inverse forward proposal PDF 1/q(g',g) = 2*e_g if not too close to the border
	    FwPrp=min(U,old_g+e_g[i])-max(0.0,old_g-e_g[i]);
	    ...
	    BwPrp=min(U,g[i]+e_g[i])-max(0.0,g[i]-e_g[i]);
            ...
            {
                // calculate old and new value of theta
                new_theta=exp(-(beta[j]+g[i]*X[j]));	//revision env1 (PdV)
            ...
            //log_old_g=log(old_g);
            //slog_g=log(g[i]);
             A= diff_log_likelihood + log(FwPrp) - log(BwPrp);		//log(odd_prior) for uniform is zero
...
//    double old_log_likelihood; // old logikelihood
    ...
    for (int j=0; j<J; j++)
    {
        ...
        // propose the new values
        u=randgen_parallel[omp_get_thread_num()].randDblExc(FwPrp)+max(0.0,old_f-e_f[j]);
        f[j]=u;
        ...
        // ratio A
        A= diff_log_likelihood + log(FwPrp) - log(BwPrp);
...
//    double old_log_likelihood; // old logikelihood
    ...
    #pragma omp parallel for SCHED_J reduction(+:log_likelihood) private(r, A, FwPrp, BwPrp, u,old_freq,diff_log_likelihood,old_log_likelihood_part,new_log_likelihood_part)
    for (int j=0; j<J; j++) //cycle over populations
    {
        for (int i=0; i<I; i++) // cycle over loci
        {
...
//    double old_log_likelihood; // old logikelihood
    ...
    for (int i = 0; i < I; i++)
    {
        ...
        if (!discarded_loci[i])
        {
            ...
            // calculate forward proposition probability =q(p',p)
//	FwPrp=min(1.0,old_mu+e_mu)-max(0.0,old_mu-e_mu);
            FwPrp=atan((1-old_mu)/e_mu[i]) - atan(abscence_pc-old_mu/e_mu[i]);
...
//	mu[i]=u;
            ...
            BwPrp=atan((1-mu[i])/e_mu[i]) - atan(abscence_pc-mu[i]/e_mu[i]);

            // calculate backward proposition probability =q(p,p')
//	BwPrp=min(1.0,mu[i]+e_mu)-max(0.0,mu[i]-e_mu);
            ...
            // normal :
            A= diff_log_likelihood + var_intensity*var_intensity*(delta[i]-old_mu)*(delta[i]-old_mu)/(2*old_mu*old_mu) - var_intensity*var_intensity*(delta[i]-mu[i])*(delta[i]-mu[i])/(2*mu[i]*mu[i])  + log(FwPrp) - log(BwPrp); //revision env1 (Pdv) --> gamma changed to var_intensity to avoid conflicts
            r=randgen_parallel[omp_get_thread_num()].randDblExc();
            // reject proposed value
            if (log(r)>A)
            {
...
//    double s; // choose type of proposal
    ...
    for (int i = 0; i < I; i++)
    {
        if (!discarded_loci[i])
        {
            ...
            // calculate forward proposition probability =q(p',p)
            //FwPrp=min(1.0,old_delta+e_delta)-max(0.0,old_delta-e_delta);
            FwPrp=atan((1-old_delta)/e_delta[i]) - atan(-old_delta/e_delta[i]);
...
//	delta[i]=u;
            ...
            BwPrp=atan((1-delta[i])/e_delta[i]) - atan(-delta[i]/e_delta[i]);

            // calculate backward proposition probability =q(p,p')
            //BwPrp=min(1.0,delta[i]+e_delta)-max(0.0,delta[i]-e_delta);
            ...
            // ratio A  to improve
            //cauchy:
            //A= diff_log_likelihood + log((old_delta-mu[i])*(old_delta-mu[i])+var_intensity*var_intensity) -  log((delta[i]-mu[i])*(delta[i]-mu[i])+var_intensity*var_intensity) + log(BwPrp)- log(FwPrp);
            // normal:
            A= diff_log_likelihood + var_intensity*var_intensity*(old_delta-delta[i])*(old_delta+delta[i]-2*mu[i])/(2*mu[i]*mu[i]) + log(FwPrp)- log(BwPrp);	//revision env1 (Pdv) --> gamma changed to var_intensity to avoid conflicts
            r=randgen_parallel[omp_get_thread_num()].randDblExc();
            // reject proposed value
            if (log(r)>A)
            {
