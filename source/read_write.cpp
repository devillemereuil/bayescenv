...
int read_input(std::ifstream& infile, string env_filename,string outcheck)
{
    ...
    while (getline(infile,line,'=') )
    {
        if (line.length()>=6 && line.substr(line.length()-6,line.length())=="[loci]") //read nb of loci
        {
            ...
            {
                cout << "Not enough memory for g" << endl;
                return 1;
            }
            try
            {
            ...
            {
                cout << "Not enough memory for mean_g"<< endl;
                return 1;
            }
            try
            {
            ...
            {
                cout << "Not enough memory for var_g"<< endl;
                return 1;
            }
            try
            {
                log_trunc=new double[I];
            }
            catch ( const std::exception & Exp )
            {
                cout << "Not enough memory for log_trunc"<< endl;
                return 1;
            }
            try
            {
            ...
            {
                cout << "Not enough memory for post_g"<< endl;
                return 1;
            }
            try
            {
            ...
            {
                cout << "Not enough memory for nb_g"<< endl;
                return 1;
            }
            try
            {
                g_included=new bool[I];
            }
            catch ( const std::exception & Exp )
            {
                cout << "Not enough memory for g_included" << endl;
                return 1;
            }
            try
            {
            ...
            {
                cout << "Not enough memory for model" << endl;
                return 1;
            }
            //end of revision ------------------------------------------------
            try
...
//selection2
            try
            {
                ...
                acc_g = new double [I];				//revision env1 (PdV)
                var_prop_alpha = new double [I];
                var_prop_g = new double[I];			//revision env1 (PdV)
                acc_freq_ancestral = new double [I];
                e_ancestral = new double [I];
		e_g = new double [I];				//revision env1 (PdV)
            }
            catch ( const std::exception & Exp )
            {
                cout << "Not enough memory for acc_rte for alpha, g or freq_ancestral "<< endl;		//revision env1 (PdV)
                return 1;
            }
		...
		env=new double[J];
            ...
            {
                cout << "Not enough memory for environmental values"<< endl;
                return 1;
            }
	    //end of revision
            if (codominant<1)
...
// initialize fst table
    ...
    if (!fstat){
      ifstream envfile(env_filename.c_str());
      assure(envfile);
...
// write check file
    if (codominant==0)
      outfile << "Mode: dominant data" << endl << endl;
    if (codominant==1)
      outfile << "Mode: codominant data" << endl << endl;

    outfile << "Upper bound for Uniform prior: " << U << endl;
    outfile << "Prior jump probability: " << prior_jump << endl;
    outfile << "Prior preference for alpha model: " << prior_pref << endl;
    outfile << "Prior mean for alpha: " << mean_prior_alpha  <<endl << endl;
    ...
    //revision env1 (PdV)
    outfile << "Raw environmental variable:" << endl;
    for (int j=0;j<J;j++)
      outfile << env[j] << " ";
    outfile << endl << "Distance-transformed (i.e. absolute value) environmental variable:" <<endl;
    for (int j=0;j<J;j++)
      outfile << X[j]<< " ";
    outfile<<endl<<endl;
    //end of revision
...
int read_input_intensity(string outname,string env_filename,string outcheck)
{
    ofstream outfile;
...
// allocate memory
    ...
    {
        cout << "Not enough memory for acc_rte alpha, g or freq_ancestral "<< endl;		//revision env1 (PdV)
        return 1;
    }
    ...
    while (getline(infile,line))
    {
        ...
        for (int i = 0; i < I; i++)
        {
            ...
            if (pop[cur_pop-1].locus[i].indiv[cur_indiv-1].intensity>max_intensity[i])
                max_intensity[i]=pop[cur_pop-1].locus[i].indiv[cur_indiv-1].intensity;
        }
    }
            ...
            for (int j=0;j<J;j++)
            {
                for (int k = 0; k < pop[j].locus[i].n; k++)
                {
                    if (max_intensity[i]>0)
                        pop[j].locus[i].indiv[k].intensity/=max_intensity[i];
                }
            }
        }
    }
...
// write check file
    ...
    outfile << "Mode: intensities" << endl << endl;
    ...
    for (int j=0;j<J;j++)
      outfile << env[j] << " ";
    outfile << endl << "Distance-transformed (i.e. squared) environmental variable:" <<endl;
    for (int j=0;j<J;j++)
      outfile << X[j]<< " ";
    outfile<<endl<<endl;
...
// write iteration index
    outfile << iter << "";				//revision env1: changed for MCMC analysis in R (PdV)
// write loglikelihood
...
// write mu and mu+delta
    for (int i=0;i<I;i++)
    {
        ...
        {
            if (max_intensity[i]!=0)
                outfile << setprecision(8) << " " << mu[i]*max_intensity[i];
            else
                outfile << setprecision(8) << " " << mu[i];

            if (max_intensity[i]!=0)
                outfile << setprecision(8) << " " << (delta[i]+mu[i])*max_intensity[i];
            else
...
// write sigma1 and sigma2
    for (int i=0;i<I;i++)
    {
        ...
        {
            if (max_intensity[i]!=0)
                outfile << setprecision(8) << " " << sigma1[i]*max_intensity[i];
            else
                outfile << setprecision(8) << " " << sigma1[i];
            if (max_intensity[i]!=0)
                outfile << setprecision(8) << " " << sigma2[i]*max_intensity[i];
            else
