...
GLOB bool is_pilot;	// boolean indicating whether we currently are running the pilot runs or the actual RJMCMC
GLOB int nb_factors;	//number of environmental factors
GLOB double U;		//Upper bound of the uniform prior
GLOB double log_U;	//Log of U
GLOB double *log_trunc;	//Array for log of erf(mean/variance) to avoid calculation at each step (see RJ update)
...
GLOB double *g; // Vector of gamma regression slopes (locus-only dependent) //revision env1 (Pdv)
GLOB double *X; //Distance-transformed environnemental variable (population dependent)	
GLOB double *env; //Raw environmental variable	
GLOB bool *g_included; //indicates if g is not null 

//parameters for the new 3model jump procedure
GLOB int *model;	//Vector containing the current model state
//model=0: beta alone; model=1: alpha + beta; model=2: beta + gamma
GLOB double prior_jump; //Prior probability that the MCMC would jump away from model 0
GLOB double prior_pref; //Given that we are jumping away from model 0, prior probability that we would choose model 1 and not 2
//Note that the prior probability of model 1 ir prior_jump*prior_pref; for model 2 it is prior_jump*(1-prior_pref)
GLOB double log_trans_prob_0_1; //Log Prior transition odds for the jumps from model 0 to 1
GLOB double log_trans_prob_0_2;//Log Prior transition odds for the jumps from model 0 to 2
GLOB double log_trans_prob_1_2;//Log Prior transition odds for the jumps from model 0 to 2

GLOB double log_sigma_prior_alpha;	//log of 1/(simga*sqrt(2*pi)	
GLOB double log_sigma_prior_g;	//log of 1/(simga*sqrt(2*pi)	
...
GLOB double *mean_g;	//revision env1: mean and variance of g proposal (PdV)
GLOB double *var_g;
GLOB double *post_g;	//posterior mean of g
GLOB int *nb_g;		//nb of time g was included in the model
...
GLOB double *max_intensity; // maximum intensity for each locus for normalisation selection2
...
GLOB double  *e_g;	//same for g, except we only stay in (0,+inf)
...
GLOB double	  *var_prop_g; //sd of the normal proposal for g			
...
GLOB double  mean_prior_alpha;
GLOB double  sd_prior_alpha;


GLOB double  sd_prior_g; //prior for g
//mean is 0

//////////////////////////////////////////
//  global functions declarations
//////////////////////////////////////////
GLOB int read_input(std::ifstream& infile,string env_filename,string outcheck);
GLOB int read_input_intensity(string outname,string env_filename,string outcheck); // selection2
...
GLOB double log_prior_g(double g);				
...
// GLOB void jump_g_model();					
// GLOB void jump_g_model_codominant();		
GLOB void jump_3model();
GLOB void jump_3model_codominant();
GLOB void jump_3modelbis_codominant();
